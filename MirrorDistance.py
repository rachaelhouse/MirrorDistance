...
class MirrorDistanceWidget(ScriptedLoadableModuleWidget):
  ...
  """
    ...
    def onApplyButton(self):
        ...
        logic.run(self.inputModelSelector.currentNode(), self.outputSelector.currentNode(),
                  self.transformSelector.currentNode(), self.distanceLabel, self.inputModelSelector2.currentNode(),
                  self.inputFiducialSelector.currentNode())
...
class MirrorDistanceLogic(ScriptedLoadableModuleLogic):
    ...
    def MirrorInput(self, inputModel, outputModel, initialTransform, distance, inputModel2, fids):
        ...
        # clean the CLI Model to Model distance model using the original surface and the mirrored registered surface as input
        # store module output in output model provided
        params = {'vtkFile1': inputModel.GetID(), 'vtkFile2': mirroredModel.GetID(), 'vtkOutput': outputModel.GetID(),
                  'distanceType': 'signed_closest_point', 'targetInFields': False}
